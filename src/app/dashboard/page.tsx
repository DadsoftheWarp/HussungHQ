...
import { Calendar, DollarSign, CheckSquare, Target, ListChecks, Utensils, Clock, ChevronRight } from "lucide-react";
import { format, isToday, isTomorrow, parseISO, startOfDay, isBefore, addDays, eachDayOfInterval } from "date-fns";
import { doesEventOccurOn } from "@/lib/calendarUtils";

const MODULES = [
  { href: "/dashboard/calendar", icon: Calendar, label: "Calendar", color: "#6366f1", desc: "Upcoming events" },
  { href: "/dashboard/budget", icon: DollarSign, label: "Budget", color: "#10b981", desc: "Track spending" },
  { href: "/dashboard/todos", icon: CheckSquare, label: "To-Do Lists", color: "#f59e0b", desc: "Tasks & lists" },
  { href: "/dashboard/goals", icon: Target, label: "Goals", color: "#ec4899", desc: "House projects" },
  { href: "/dashboard/chores", icon: ListChecks, label: "Chores", color: "#8b5cf6", desc: "Weekly chores" },
  { href: "/dashboard/meals", icon: Utensils, label: "Meal Planner", color: "#14b8a6", desc: "Weekly meals" },
];
...
export default function DashboardPage() {
  ...
  const [upcomingEvents, setUpcomingEvents] = useState<(CalendarEvent & { occurrenceDate: string })[]>([]);
      ...
      <div className="rounded-2xl border overflow-hidden"
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b"
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="font-semibold text-sm" style={{ color: "var(--foreground)" }}>Upcoming</p>
          </div>
        ...
        {upcomingEvents.length === 0 ? (
          ...
          <div className="divide-y" style={{ borderColor: "var(--card-border)" }}>
            {upcomingEvents.map((event) => (
              <Link key={`${event.id}-${event.occurrenceDate}`} href="/dashboard/calendar"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {!event.allDay && event.time && (
                      <p className="text-xs flex items-center gap-0.5" style={{ color: "var(--muted-foreground)" }}>
